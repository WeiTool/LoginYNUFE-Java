...
public class LoginLogout {
    ...
    public static void performLogin(Account account, OnLoginListener listener) {
        NETWORK_EXECUTOR.execute(() -> {
            // 后台执行网络请求
            Map<String, Object> result = login(account.getUsername(), account.getPassword(), "", true);
    ...
    public static void performLogout(Account account, OnLogoutListener listener) {
        NETWORK_EXECUTOR.execute(() -> {
            // 后台执行网络请求
    ...
    public interface OnLoginListener {
        void onLoginResult(Map<String, Object> result);
    }
    ...
    private static String buildLoginUrl(String username, String hmd5, String clientIp, String info, String chksum) throws UnsupportedEncodingException {
        ...
        return SRUN_PORTAL_API + "?callback=sdu"
                + "&action=login"
                + "&username=" + URLEncoder.encode(username, "UTF-8") // 用户名编码
                + "&password={MD5}" + hmd5 // 添加MD5前缀的密码
                + "&ac_id=" + AC_ID
                + "&ip=" + URLEncoder.encode(clientIp, "UTF-8") // IP编码
                + "&info=" + URLEncoder.encode(info, "UTF-8") // info编码
                + "&chksum=" + chksum
                + "&n=" + N
                + "&type=" + UTYPE
    ...
    private static String buildLogoutUrl(String username, String clientIp) throws UnsupportedEncodingException {
        return SRUN_PORTAL_API + "?callback=sdu"
                ...
                + "&ip=" + URLEncoder.encode(clientIp, "UTF-8")
                + "&ac_id=" + AC_ID
                + "&_=" + (System.currentTimeMillis() / 1000); // 时间戳参数
    }
    ...
    public static Map<String, String> getChallenge(String username, String clientIp) throws IOException {
        ...
        try {
            ...
            String url = GET_CHALLENGE_API + "?callback=sdu&username=" + URLEncoder.encode(username, "UTF-8")
                    + "&ip=" + URLEncoder.encode(clientIp, "UTF-8")
                    + "&_=" + timestamp;
